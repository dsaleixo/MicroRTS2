...
//                    System.out.println("    " + pos%w + "," + pos/w);
    ...
    public Pair<Integer, Integer> findPathToPositionInRange2(Unit start, int targetpos, int range, GameState gs) {
        PhysicalGameState pgs = gs.getPhysicalGameState();
        int w = pgs.getWidth();
        int h = pgs.getHeight();
        if (free==null || free.length<w*h) {
...
//                    System.out.println("    " + pos%w + "," + pos/w);
                ...
                if (last == pos+w) return new Pair<>(x, y);;
                if (last == pos-1) return new Pair<>(x, y);;
                if (last == pos-w) return new Pair<>(x, y);;
                if (last == pos+1) return new Pair<>(x, y);;
                return null;
            }
